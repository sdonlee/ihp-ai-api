...
class VideoIO:
    def __init__(self, video_path:str="../video_test/test_videos/test/videoplayback.mp4", target_size:tuple=(320,240), DEBUG:bool=False):
        ...
        self.__time = dict()
        self.__time["video_rd_"] = 0.
        self.__time["video_rsz"] = 0.
        self.__time["video_wrt"] = 0.
        
        self.__path_vid_ori = video_path
        self.__path_vid_det = self.__gen_path_vid_det()
        ...
        self.nfrm = 0
        self.__size_ori = (0,0) 
        self.__size_out = target_size
        
    def __gen_path_vid_det(self):
        video_path = self.__path_vid_ori 
        return f"../result/{video_path.split('/')[-1]}"
        
    def print_time(self):
        for item in list(self.__time.keys()):
            print(f" - PROCESS TIME for {item} : {self.__time[item]:.3f} sec ")
    ...
    def video_read(self):
        ...
        self.vid_ori, self.fps = video_read(self.__path_vid_ori)
        self.nfrm = self.vid_ori.shape[0]
        self.__size_ori = self.vid_ori.shape[1:3]
        # self.target_size = (320,240)
    
        if self.__DEBUG:
            t2 = time.time()
            print(f" - VIDEO Info:")
            print(f" - >>> {self.__path_vid_ori}") 
            print(f" - >>> {self.vid_ori.shape}")
            print()
            print(f" - >>> {t2-t1:.2f} secs - Time for Video Load")
            self.__time["video_rd_"] = t2-t1
    ...
    def video_resize(self):
        ...
        if self.__DEBUG:
            t1 = time.time()
        if self.__size_out[0]==0:
            self.vid_rsz = self.vid_ori
        else:
            self.vid_rsz = vid_resize(self.vid_ori, self.__size_out)
    ...
    def video_write(self):
        ...
        if self.__DEBUG:
            print(f" - VIDEO Write:")
            print(f" - >>> {self.__path_vid_det}") 
            print(f" - >>> {self.vid_out.shape}")
            print()
            print(f" - >>> {t2-t1:.2f} secs - Time for Video Write")
            self.__time["video_wrt"] = t2-t1
            
    # def delete_video_in(self):
    #     if os.__path.exists(self.__path_vid_ori):
    #         os.remove(self.__path_vid_ori)
    
    # def delete_video_out(self):
    #     if os.__path.exists(self.__path_vid_det):
    #         os.remove(self.__path_vid_det)
...
class EasyNeg(VideoIO):
    def __init__(self, video_path:str='', target_size:tuple = (320,240), DEBUG:bool=False):
        ...
        self.__DEBUG = DEBUG
        self.__time["easyneg_drk"] = 0.
        self.__time["easyneg_mtl"] = 0.
    ...
    def __get_info(self):
        ...
        self.__time_range = config['NEG']['TIME_RANGE']
    ...
    def analyze_drk(self):
        ...
        f_vid = feat_vid(f_frm, "avr", val_ = self.__time_range)

        if self.__DEBUG:
            t2 = time.time()
            self.__time['easyneg_drk'] = t2-t1
...
class ObjDet(VideoIO):
    def __init__(self, video_path:str, prj_name:str="TST001DMO", label_fake:bool=True, save_video:bool=True, DEBUG:bool=False):
        ...
        self.__DEBUG = DEBUG
        self.__time["objdet_det"] = 0.
        self.__time["objdet_drw"] = 0.
        self.__time["objdet_wrt"] = 0.
        ...
        self.__path_out = f"{self.default_path}/../../result" #'/'.join(model_path.split('/')[:-6] +['result'])
        videoname = self.__path_vid_ori.split('/')[-1]
        self.__path_vid_det= self.__path_out +'/'+ 'det_'+videoname
    ...
    def save_vid(self):
        ...
        video_write(self.vid_out, self.__path_vid_det)
        if self.__DEBUG:
            t2 = time.time()
            self.__time["objdet_wrt"] = t2-t1
